...
- **Backtest realism**: Runner uses `FillModel(prob_slippage=0.5)` for 50% chance of 1-tick slippage on fills. Seed configurable via `--seed` (default 42, `--seed 0` = random). Exit TP/SL checks use bar high/low (not close) for realistic intra-bar fills. `MakerTakerFeeModel` fees are low (0.002%) — spreads are the real cost for FX.
